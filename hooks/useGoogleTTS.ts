...
export function useGoogleTTS({
  ...
  const speak = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        if (response.ok && data.success && data.audio) {
          console.log('✅ Using Google Cloud TTS (Neural2)');
    ...
    [gradeLevel, language, isSpeaking, onStart]
  );
  ...
  const fallbackToWebSpeech = useCallback(
    (text: string): Promise<void> => {
      return new Promise((resolve, reject) => {
        try {
          ...
          utterance.rate = getWebSpeechRate(gradeLevel);
          utterance.pitch = getWebSpeechPitch(gradeLevel);
    ...
    [gradeLevel, language, onEnd, onError]
  );
