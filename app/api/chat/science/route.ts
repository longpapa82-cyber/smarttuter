...
export async function POST(req: NextRequest) {
  try {
    ...
    const complexityAnalysis = classifyComplexity(message, 'science');
    console.log(`[Complexity] Science question: "${message.substring(0, 50)}" → ${complexityAnalysis.complexity} (confidence: ${complexityAnalysis.confidence})`);
    ...
    try {
      ...
      if (retrievedContext.content.length > 0) {
        ...
        if (avgConfidence > 0.9 && retrievedContext.content.length >= 1) {
          ...
          ragDirectAnswer = `📚 **검증된 과학 교육 자료를 바탕으로 답변드려요:**

${contentToUse.join('\n\n---\n\n')}

💡 더 궁금한 점이 있으시면 언제든 질문해주세요!`;

          console.log(`[Science RAG Direct KO] High confidence (${avgConfidence.toFixed(2)}) - answering without API or filter`);

          // Cache this RAG-based answer
          responseCache.set(message, 'science', gradeStr, ragDirectAnswer);
          ...
          console.log(`[Science RAG] Low confidence (${avgConfidence.toFixed(2)}) - continuing to Enhanced Filter`);
        }
      }
    } catch (error) {
    ...
    // ⚠️ ENHANCED FILTER DISABLED: Too strict, causing false negatives on valid Science questions
    ...
    logFilterDecision('science', message, enhancedFilterResult);

    if (!enhancedFilterResult.shouldRespond) {
    */
    // Enhanced Filter bypassed - always allow Science questions
    ...
    logGradeLevelDecision('science', message, userProfile.gradeLevel, gradeLevelResult);
...
📝 Gemini API는 무료 tier에서도 충분한 할당량을 제공합니다!`;
    ...
    const currentDifficulty = await getCurrentDifficulty(userId, 'science', userProfile.gradeLevel);
