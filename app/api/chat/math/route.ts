...
export async function POST(req: NextRequest) {
  try {
    ...
    // 🚀 Phase 1.5: Classify question complexity (NEW - no API call)
    const complexityAnalysis = classifyComplexity(message, 'math');
    console.log(`[Complexity] Math question: "${message.substring(0, 50)}" → ${complexityAnalysis.complexity} (confidence: ${complexityAnalysis.confidence})`);
...
💡 더 궁금한 점이 있으시면 언제든 질문해주세요!`;
          ...
          console.log(`[Math RAG Direct KO] High confidence (${avgConfidence.toFixed(2)}) - answering without API or filter`);
          ...
          console.log(`[Math RAG] Low confidence (${avgConfidence.toFixed(2)}) - continuing to Enhanced Filter`);
        }
      }
    } catch (error) {
...
📝 Gemini API는 무료 tier에서도 충분한 할당량을 제공합니다!`;
    ...
    // Phase 5: RAG retrieval moved to BEFORE Enhanced Filter (see lines 148-240)
    ...
    const currentDifficulty = await getCurrentDifficulty(userId, 'math', userProfile.gradeLevel);
    ...
    if (complexityAnalysis.complexity === 'simple') {
      ...
      const basePrompt = generateEnhancedSystemPrompt({
        ...
        enableSocraticMode: false, // Disable for now to avoid conflicts
...
튜터:`;
            ...
            extractConceptId(message, 'math', gradeStr)
              .then(conceptId => {
                const learningEvent: LearningEvent = {
                  userId,
                  eventType: 'question_attempt',
                  subject: 'math',
                  conceptId, // AI-extracted concept (e.g., "math_linear_equations")
