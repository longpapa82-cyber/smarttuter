...
export async function POST(req: NextRequest) {
  try {
    ...
    const cachedAnswer = responseCache.get(message, 'english', gradeStr);
    ...
    const complexityAnalysis = classifyComplexity(message, 'english');
    console.log(`[Complexity] English question: "${message.substring(0, 50)}" → ${complexityAnalysis.complexity} (confidence: ${complexityAnalysis.confidence})`);

    // 🚀 Phase 2: Quick keyword-based classification (no API call)
    const quickClassification = quickClassify(message, 'english');
    ...
    if (quickClassification && !quickClassification.isOnTopic) {
      ...
      const redirectStream = new ReadableStream({
        start(controller) {
          const redirectMsg = `📚 **영어 튜터**에서 도와드려요! 영어 관련 질문을 해주세요.`;
    ...
    if (isObviouslyOffTopic(message, 'english')) {
      ...
      const quickFilterStream = new ReadableStream({
        start(controller) {
          const redirectMsg = `📚 영어 관련 질문은 **English Park**에서 도와드릴 수 있어요!
...
💡 더 궁금한 점이 있으시면 언제든 질문해주세요!`;
          ...
          console.log(`[English RAG Direct KO] High confidence (${avgConfidence.toFixed(2)}) - answering without API or filter`);

          // Cache this RAG-based answer
          responseCache.set(message, 'english', gradeStr, ragDirectAnswer);
          ...
          console.log(`[English RAG] Low confidence (${avgConfidence.toFixed(2)}) - continuing to Enhanced Filter`);
        }
      }
    } catch (error) {
    ...
    // ⚠️ ENHANCED FILTER DISABLED: Too strict, causing false negatives on valid English questions
    ...
    logFilterDecision('english', message, enhancedFilterResult);

    if (!enhancedFilterResult.shouldRespond) {
    */
    // Enhanced Filter bypassed - always allow English questions
    ...
    logGradeLevelDecision('english', message, userProfile.gradeLevel, gradeLevelResult);
...
튜터:`;
              ...
              const streamIterator = await vertexAIClient.generateContentStream(
                ...
                {
                  temperature: 0.7,
                  maxTokens: 4096, // Increased from 3072 for comprehensive answers
                }
              );
            ...
            if (fullResponse.trim()) {
              ...
              responseCache.set(message, 'english', gradeStr, fullResponse);
            }

            // Track learning event (Phase 8: Progress tracking integration)
            // Extract meaningful concept from the conversation using AI
            const responseTime = Math.round((Date.now() - startTime) / 1000); // seconds

            extractConceptId(message, 'english', gradeStr)
              .then(conceptId => {
                const learningEvent: LearningEvent = {
                  userId,
                  eventType: 'conversation_turn',
                  subject: 'english',
                  conceptId, // AI-extracted concept (e.g., "english_past_tense", "english_vocabulary")
