 ...
 * - 사회 튜터: 한국어 + Push-to-Talk + 수동 시작 (수학과 동일)
 */
 ...
 * 과목별 음성 입력 안내 메시지
 */
export function getVoiceInputGuideMessage(subject: 'english' | 'math' | 'science' | 'social-studies'): string {
  if (subject === 'math') {
  ...
  if (subject === 'social-studies') {
    return '🎤 버튼을 길게 눌러 사회 질문을 말씀해주세요. (한국어 지원)';
  }
...
export function getVoiceSettingsDescription(
  ...
  if (subject === 'social-studies') {
    return {
      title: '사회 튜터 음성 설정',
      description: '사회 용어와 한국어 설명에 최적화된 설정입니다.',
      features: [
        '✓ 한국어 음성 인식',
        '✓ Push-to-Talk 모드 (정확한 입력)',
        '✓ 수동 시작 (준비되면 시작)',
