...
/**
 * 과목별 기본 설정 가져오기
 * @param subject 과목 ('english' | 'math')
 * @returns 해당 과목에 최적화된 음성 설정
 */
export function getSubjectDefaultSettings(
  subject: 'english' | 'math'
): VoiceSettingsConfig {
  return subject === 'math' ? MATH_TUTOR_DEFAULTS : ENGLISH_TUTOR_DEFAULTS;
}
 ...
 */
export function shouldAutoStartVoice(subject: 'english' | 'math'): boolean {
 ...
 * 과목별 음성 입력 안내 메시지
 */
export function getVoiceInputGuideMessage(subject: 'english' | 'math'): string {
  if (subject === 'math') {
